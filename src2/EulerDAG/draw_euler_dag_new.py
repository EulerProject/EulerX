...
cur.execute("select distinct concept1,concept2 from test2 where relation = 'disjoint'")
...
cur.execute("select distinct concept1,concept2 from test2 where relation = 'is_included_in'")
results = cur.fetchall()
add_list = []
for result in results:
...
cur.execute("select distinct concept1,concept2 from test2 where relation = 'includes'")
results = cur.fetchall()
add_list = []
for result in results:
...
cur.execute("select distinct concept1,concept2 from test2 where relation = 'overlaps'")
results = cur.fetchall()
...
# draw the combination nodes and the edges among them
comb_node_list_str = []
for i in range(len(comb_node_list)):
    comb_node_list_str.append("")
for i in range(len(comb_node_list)):
    for member in comb_node_list[i]:
        comb_node_list_str[i] = comb_node_list_str[i] + member + ","
    comb_node_list_str[i] = comb_node_list_str[i][:-1]

print "comb_node_list_str =", comb_node_list_str        
for member_str in comb_node_list_str:
    fDAG.write("\"" + member_str + "\"" + " [color = grey];\n")
...
for member_str in comb_node_list_str:   
    for member in comb_node_list:
        ...
        if member_str == temp_str:
            for letter in member:
                fDAG.write("\"" + letter + "\"" + " -> " + "\"" + member_str + "\"" + " [color = grey];\n")
...
for member in final_list:
    ...
    while i < len(member):
        ...
        if tuple_not_in_list(t, overlaps_list):
            edge_string = "\"" + member[-i] + "\"" + " -> " + "\"" + member[-i-1] + "\"" + ";\n"
