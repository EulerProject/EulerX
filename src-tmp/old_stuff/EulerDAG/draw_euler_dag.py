...
for result in results:
    add_list.append(result[0] + result[1])
    add_list.append(result[1] + result[0])
    overlaps_list.append(result[0] + result[1])
    overlaps_list.append(result[1] + result[0])
for i in range(len(add_list)-1):
    for j in range(i+1,len(add_list)):
        if add_list[i] == add_list[j]:
            drop_list.append(add_list[i])
...
for i in range(len(comb_node_list)-1):
    for j in range(i+1,len(comb_node_list)):
        if check_same_string(comb_node_list[i],comb_node_list[j]):
...
for member in comb_node_list:
    fDAG.write("\"" + member + "\"" + " [color = grey];\n")
    for letter in member:
        fDAG.write("\"" + letter + "\"" + " -> " + "\"" + member + "\"" + " [color = grey];\n")
...
for member in final_list:
    i = 1
    while i < len(member):
        s = member[-i] + member[-i-1]
        print "s = ", s
        if not_in_list(s,overlaps_list):
            edge_list.append(s)
        i = i + 1
remove_duplicate_string(edge_list)
print "edge_list = ", edge_list

for member in edge_list:
    fDAG.write("\"" + member[0] + "\"" + " -> " + "\"" + member[1] + "\"" + ";\n")
